...
$allowedFields = ['name', 'project_count', 'created_at', 'sub_status'];
...
// Récupérer les artistes avec statistiques et tri
$query = "SELECT u.*, 
...
function getSortURL($field, $currentSort, $currentOrder) {
    ...
    return "artists.php?sort=$field&order=$newOrder";
}

$pageTitle = 'Master Control - Artistes';
?>
<!DOCTYPE html>
<html lang="fr">
<head>
...
<body>
    ...
    <main class="main-content">
        <header class="mb-16">
            <h2 class="text-6xl font-black tracking-tighter leading-none">Master <span class="text-yellow-500">Artistes</span></h2>
            <p class="text-gray-500 mt-4 text-xl">Surveillance globale des créateurs et contrôle des flux de contenus.</p>
        </header>
            ...
            <div class="overflow-x-auto p-4">
                <table class="super-table text-left">
                    <thead>
                        <tr>
                            <th class="px-8 py-4">
                                <a href="<?= getSortURL('name', $sortField, $sortOrder) ?>" class="hover:text-yellow-500 transition-colors flex items-center gap-2">
                                    Créateur
                                    <i class="fas fa-sort<?= $sortField === 'name' ? ($sortOrder === 'ASC' ? '-up' : '-down') : '' ?> opacity-50 text-[10px]"></i>
                                </a>
                            </th>
                            <th class="py-4">
                                <a href="<?= getSortURL('project_count', $sortField, $sortOrder) ?>" class="hover:text-yellow-500 transition-colors flex items-center gap-2">
                                    Stats Projets
                                    <i class="fas fa-sort<?= $sortField === 'project_count' ? ($sortOrder === 'ASC' ? '-up' : '-down') : '' ?> opacity-50 text-[10px]"></i>
                                </a>
                            </th>
                            <th class="py-4">Accès Système</th>
                            <th class="py-4">
                    ...
                    <tbody>
                            ...
                            <tr>
                                <td class="px-8">
                                    <div class="flex items-center gap-5">
                                        <div class="w-12 h-12 rounded-2xl bg-yellow-500/10 flex items-center justify-center border border-yellow-500/20">
                                            <i class="fas fa-user text-yellow-500/50 text-xl"></i>
                                        </div>
                                        <div>
                                            <div class="font-black text-white tracking-tight"><?= htmlspecialchars($a['name']) ?></div>
                                            <div class="text-[10px] text-gray-500 font-bold uppercase tracking-widest"><?= htmlspecialchars($a['email']) ?></div>
                                        </div>
                                    </div>
                                </td>
                                <td>
                                    <div class="flex items-center gap-2">
                                        <span class="text-2xl font-black text-white"><?= $a['project_count'] ?></span>
                                        <span class="text-[9px] font-black uppercase text-gray-600 tracking-tighter">Projets</span>
                                    </div>
                                </td>
                                <td>
                                    <div class="flex items-center gap-2">
                                ...
                                <td>
                                    <span class="badge <?= $subClass ?>">
                                        <?= $subStatus === 'active' ? 'Membre Pro' : ($subStatus === 'expired' ? 'Expiré' : 'Standard') ?>
                                    </span>
                                </td>
                                ...
                                <td>
                                    <form method="POST" onsubmit="return confirm('Attention : Action de contrôle de compte. Confirmer ?')">
                                    ...
                                    <form method="POST" onsubmit="return confirm('Offrir 1 an d\'abonnement Pro à cet artiste ?')" class="mt-2">
                                        <input type="hidden" name="user_id" value="<?= $a['id'] ?>">
                        ...
                        <?php if (empty($artists)): ?>
                            <tr><td colspan="6" class="text-center py-24 text-gray-600 font-bold uppercase tracking-[4px]">No Creators Found</td></tr>
                        <?php endif; ?>
                    </tbody>
                </table>
            </div>
