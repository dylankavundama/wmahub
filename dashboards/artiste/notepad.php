...
<head>
    ...
    <style>
        ...
        .btn-primary { display: flex; align-items: center; gap: 10px; padding: 12px 24px; background: linear-gradient(135deg, #ff6600 0%, #ff8c00 100%); color: #fff; border-radius: 14px; font-weight: 700; transition: all 0.3s ease; font-size: 0.85rem; text-transform: uppercase; letter-spacing: 0.5px; }
        .btn-primary:hover { transform: translateY(-2px); box-shadow: 0 8px 20px rgba(255, 102, 0, 0.3); }
    </style>
</head>
<body>
        ...
        <main class="flex-1 p-6 lg:p-12">
            <header class="mb-12 flex flex-col md:flex-row md:items-end justify-between gap-6">
                <div>
                    ...
                    <h2 class="text-4xl lg:text-5xl font-black mb-2 tracking-tighter text-white">Bloc <span class="text-orange-500 text-transparent bg-clip-text bg-gradient-to-r from-orange-400 to-orange-600">Note</span></h2>
                    <p class="text-gray-500 font-medium">Gérez vos lyrics, idées et inspirations en un seul endroit.</p>
                </div>
            ...
            <div class="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6">
                <?php foreach ($notes as $note): ?>
                    <div class="glass-card flex flex-col">
                        ...
                        <h3 class="font-bold text-lg mb-2 truncate text-white"><?= htmlspecialchars($note['title'] ?: 'Sans Titre') ?></h3>
                        <p class="text-gray-500 text-sm line-clamp-3 mb-6 flex-1">
                            <?= $note['content'] ? htmlspecialchars(substr($note['content'], 0, 150)) . (strlen($note['content']) > 150 ? '...' : '') : '<i class="text-gray-700 italic">Aucun contenu...</i>' ?>
                        </p>
                        <div class="flex items-center justify-between pt-4 border-t border-white/5">
                            ...
                            <a href="note_editor.php?id=<?= $note['id'] ?>" class="text-orange-500 font-bold text-[10px] uppercase tracking-widest flex items-center gap-2 hover:gap-3 transition-all">
                ...
                <?php if (empty($notes)): ?>
                    <div class="col-span-full py-20 text-center glass-card border-dashed">
                        <p class="text-gray-500 uppercase font-black tracking-widest text-[10px] mb-6">Vous n'avez pas encore de notes</p>
                        <button onclick="createNewNote()" class="text-orange-500 font-bold hover:underline uppercase text-xs tracking-widest">Créer ma première note</button>
                    </div>
                <?php endif; ?>
            </div>
        </main>
