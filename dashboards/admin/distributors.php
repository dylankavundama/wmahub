...
// Récupérer les distributeurs avec leur dernier abonnement
$query = "SELECT u.*, 
...
<head>
    ...
    <title>WMA HUB - Gestion Distributeurs</title>
...
<body>
    ...
    <main class="main-content">
        <header class="mb-12">
            <h2 class="text-4xl font-black tracking-tighter">Gestion <span class="text-orange-500">Distributeurs</span></h2>
            <p class="text-gray-400 mt-2">Gérez les accès et suivez les abonnements des distributeurs partenaires.</p>
        </header>
                ...
                <table class="admin-table text-left">
                    ...
                    <tbody>
                            ...
                            <tr>
                                <td class="px-8">
                                    <div class="flex items-center gap-4">
                                        <div class="w-10 h-10 rounded-xl bg-white/5 flex items-center justify-center border border-white/10 overflow-hidden">
                                        ...
                                        <div>
                                            <div class="font-bold text-white"><?= htmlspecialchars($d['distributor_name'] ?: $d['name']) ?></div>
                                            <div class="text-[9px] text-gray-500 uppercase tracking-tighter"><?= htmlspecialchars($d['email']) ?></div>
                                        </div>
                                    </div>
                                </td>
                                <td class="text-sm"><?= htmlspecialchars($d['distributor_city'] ?: 'N/A') ?></td>
                                <td>
                                ...
                                <td>
                                    <span class="status-badge <?= $isActive ? 'bg-blue-500/10 text-blue-400' : 'bg-red-500/10 text-red-400' ?>">
                                        <?= $isActive ? 'AUTORISÉ' : 'SUSPENDU' ?>
                                    </span>
                                </td>
                                <td>
                                    <form method="POST" onsubmit="return confirm('Voulez-vous vraiment <?= $isActive ? 'suspendre' : 'activer' ?> ce distributeur ?')">
                        ...
                        <?php if (empty($distributors)): ?>
                            <tr><td colspan="6" class="text-center py-20 text-gray-500 italic">Aucun distributeur enregistré.</td></tr>
                        <?php endif; ?>
                    </tbody>
                </table>
            </div>
